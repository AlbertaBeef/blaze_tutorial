...
Each branch adds an accelerator-specific flow. `mx3`, `qcs6490`, `axelera`, and `deepx` branch from `2023.1` HEAD.
...
| `deepx` | DeepX M1 | `deepx/` | DX-COM compiler (`deepx_flow.py`), TFLite → ONNX → `.dxnn` NPU binary |
...
### DeepX M1 Flow (`deepx/`) — branch `deepx`
- `deepx_flow.py` — Central compilation script. Generates DX-COM JSON config from model registry, invokes `dx_com` CLI via subprocess. Supports `--calib_method minmax|ema`, `--calib_num`, `--shrink`.
- `deploy_models.sh` — Batch compilation of all 13 models (v0.07 hand + v0.10 hand/face/pose).
- `models/get_tflite_models.sh` / `models/convert_models.sh` — TFLite download and ONNX conversion (tf2onnx in isolated venv).
- Calibration: 9 pre-computed `.npy` datasets placed in `deepx/` directory, referenced in generated JSON configs.
- Output: `compiled_<model_name>/` directories containing `.dxnn` binaries.
- Key difference from other flows: DX-COM is a CLI tool, not a Python SDK. `deepx_flow.py` generates JSON configs and calls `dx_com` via subprocess.

### Key Dependencies
Python: `torch`, `pytorch_nndct`, `cv2`, `numpy`, `xir`, `vitis_ai_library` (Vitis-AI); `memryx` SDK (MX3); `qai_hub` (Qualcomm); `axelera` Voyager SDK (Metis); `dx_com` CLI + `numpy` (DeepX).
...
### DeepX M1 (branch `deepx`)
```bash
git checkout deepx
cd deepx
bash models/get_tflite_models.sh                # download TFLite models
cd models && bash convert_models.sh && cd ..    # convert TFLite to ONNX
# place calib_*.npy files in deepx/ directory
python3 deepx_flow.py --name palm_detection_lite --model models/palm_detection_lite.onnx --resolution 192  # single model
bash deploy_models.sh                           # compile all 13 models for DeepX M1 NPU
```
