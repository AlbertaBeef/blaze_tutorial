...
do
	...
	model_size=${model_array[2]}

	echo python3 memryx_flow.py --arch mx3 --name ${model_name} --model ${model_file} --resolution ${model_size} --process all

	python3 memryx_flow.py --arch mx3 --name ${model_name} --model ${model_file} --resolution ${model_size} --process all | tee deploy_${model_name}.log
